...
#cnxn = pyodbc.connect('DRIVER={SQL Server};SERVER=localhost\sqlexpress;DATABASE=gbtest;UID=sa;PWD=Ytymf1991')
cnxn = pyodbc.connect('DRIVER={SQL Server};SERVER=localhost\\SQLEXPRESS;DATABASE=gbtest;UID=sa;PWD=123qweasd')
log.debug(cnxn)
...
DMM_set_sleep_times = 5 #30 # default value is 30
 ...
 #               loadsapdata.load_AEWA(cnxn, cursor)
  #              self.wfile.write('OK')
   #         except:
    ...
    def do_POST(self):
        ...
        if sStr1[1] == "keycodemanagement":
            if "post" in sStr1[2]:
                ...
                data = data[data.index('Level2WBS'):]
                data = data.split('&')
                if  'all' in str(data[0]):
                    data = 'all'
                else:
                    for i in range(len(data)):
                        data[i] = data[i][ data[i].index('AE-') : ]
                level2_status = 0
                self.read_wbs_info_by_level2wbs(data)
                level2_status = 1
                log.info("level2_status = %d",level2_status)
        if sStr1[1] == "accountManage":
            if sStr1[2] == "admin":
    ...
    def read_wbs_info_by_level2wbs(self,level2wbslist):
        # 1: used in keycodemanagement
        # 2: search database by level2wbs
        table1='level2wbs_info'
        list_1=[]
        list_1.append(self.add_space(wbs4)) # wbs
        list_1.append(self.add_space(wbs1))
        list_1.append(self.add_space(wbs2))
        list_1.append(self.add_space(wbs3))
        list_1.append(self.add_space(wbs5))
        ...
        if level2wbslist == 'all':
            cursor.execute("select [WBS ID], [System Status], [User Status],[Key Code]  from AEWA_info order by [WBS ID]")
            rows = cursor.fetchall()
            # construct a string to match SQL Server Code syntax
            for row in rows:
                strlevel2wbs += "'" + str(row[0]) + "'"  + ", "
            strlevel2wbs = strlevel2wbs[:-2]
        else:
            # construct a string to match SQL Server Code syntax
            for i in range(len(level2wbslist)):
                strlevel2wbs += '\'' + level2wbslist[i] + '\'' + ', '

            strlevel2wbs = strlevel2wbs[:-2]
            # read AEWA_info database
            cursor.execute("select [WBS ID], [System Status], [User Status],[Key Code]   from AEWA_info  where [WBS ID] in (%s) order by [WBS ID]" %strlevel2wbs)
            rows = cursor.fetchall()

        # read wbsinfo database
        cursor.execute("select %s from %s where [HTS Assigned WBS] in (%s) order by [HTS Assigned WBS]" % (items, table1, strlevel2wbs))
        srows = cursor.fetchall()

        # construct an dict, wbs as key, info list as data
        dict_srows = {}
        for i in range(len(srows)):
            dict_srows[str(srows[i][0])] = list(srows[i][1:])

        for row in rows:
            cur_level2_wbs = str(row[0])
            item_one_row = []
            try:
                item_one_row.append(str( dict_srows[cur_level2_wbs][0] )) #'Aero Network'
                item_one_row.append(str( dict_srows[cur_level2_wbs][1] )) #'Aero Activity Num'
                item_one_row.append(str( dict_srows[cur_level2_wbs][2] )) # 'Aero Key Code'
                item_one_row.append(cur_level2_wbs)  # [WBS ID]
                item_one_row.append(str(dict_srows[cur_level2_wbs][3])) #'HTS WBS Description'
                item_one_row.append(str(dict_srows[cur_level2_wbs][4])) # 'WBS Person Responsible'
                item_one_row.append(str(row[1])) # 'System Status'
                item_one_row.append(str(row[2])) # 'User Status'
                item_one_row.append('Active')
                cur_result.append(item_one_row)
            except KeyError,e:
                #log.warning('Not Found WBS: %s',cur_level2_wbs)
                item_one_row.append('NA') #'Aero Network'
                item_one_row.append('NA') #'Aero Activity Num'
                item_one_row.append(str(row[3])) # 'Aero Key Code'
                item_one_row.append(cur_level2_wbs)  # [WBS ID]
                item_one_row.append('NA') #'HTS WBS Description'
                item_one_row.append('NA') # 'WBS Person Responsible'
                item_one_row.append(str(row[1])) # 'System Status'
                item_one_row.append(str(row[2])) # 'User Status'
                item_one_row.append('Active')
                cur_result.append(item_one_row)
    ...
    # ---------------------------------------------------------------------
    def proc_cn43n(self):
        log.info('proc_cn43n is called');
        filename = dataFilePath + "CN43N_"
        currentyear =int(time.strftime("%Y"))
        currentMonth =int(time.strftime("%m"))
        filename = filename + str(currentMonth) + "_" + str(currentyear)+".xlsx"
        log.info(filename)
        fileExist = os.path.exists(filename)
        sleepCount = DMM_set_sleep_times # sleep 30 times, 2 minutes every time
        if(fileExist == False):
            os.system(batFileName+ " \"CN43N\" \"" )
            log.info(batFileName+ " \"CN43N\" \"" )
            while(fileExist == False and sleepCount>=1):
                time.sleep(DMM_set_sleep_one_cycle)
                sleepCount = sleepCount - 1
                fileExist = os.path.exists(filename)
                log.debug("fileExist = %d, sleepCount = %d",fileExist,sleepCount)
            if fileExist == False:
                #self.wfile.write('ERR,the CN43N data file cannot be generated correctly.')
                log.debug("ERR")
                return
        try:
            loadsapdata.load_AEWA(cnxn, cursor,filename)
            cur_status = 1
            #self.wfile.write('OK')
        except:
            cur_status = 0
            #self.wfile.write('ERR')
        log.info('proc_cn43n is returned')
        return cur_status
